...
placeholder.subheader('Pridicting Price')

place = st.empty()
place.image('https://www.lit.edu.in/wp-content/uploads/2025/08/Search.gif',width = 100)

if price>0:
    for i in range(100):
        time.sleep(0.05)
        progress_bar.progress(i+1)
    body = (f'Predicted Median House Price: ${round(price,2)} thousand dollors')
    placeholder.empty()
    place.empty()
    # st.subheader(body)
